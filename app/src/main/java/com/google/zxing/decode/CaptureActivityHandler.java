...
public final class CaptureActivityHandler extends Handler {
	...
	/**
	 * Really responsible scan task kernel thread
	 */
	...
	public CaptureActivityHandler(CaptureActivity activity,
		...
		// Turn on the camera preview screen
	...
	public void handleMessage(Message message) {
		switch (message.what) {
			case R.id.restart_preview: // Ready for the next scan
			...
			case R.id.launch_product_query:
				...
				/**
				 * This code is zxing project group wants to open a browser with the chrome browser
				 * url
				 */
	...
	/**
	 * To complete a scan, just need to call this method
	 */
	private void restartPreviewAndDecode() {
		if (state == State.SUCCESS) {
			state = State.PREVIEW;

			// Send a message to decode decodeThread bound handler (DecodeHandler)
			cameraManager.requestPreviewFrame(decodeThread.getHandler(),
					R.id.decode);
